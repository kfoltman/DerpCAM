...
class Gcode(object):
   ...
   def move_z(self, new_z, old_z, tool):
      if new_z == old_z:
         return
      if new_z < old_z:
         ...
         self.rapid(z=new_z)
   def ramped_move_z(self, new_z, old_z, subpath, slope):
      if new_z >= old_z:
         self.rapid(z=new_z)
         return
...
def pathToGcode(gcode, path, safe_z, start_depth, end_depth, doc, tabs, tab_depth):
            ...
            if newz != curz:
               if newz < curz:
                  if not is_tab and prev_depth < curz:
                     gcode.move_z(prev_depth, curz, p.tool)
                     curz = prev_depth
                  gcode.feed(p.tool.hfeed)
                  gcode.ramped_move_z(newz, curz, subpath, slope)
               else:
                  gcode.move_z(newz, curz, p.tool)
               curz = newz
...
class Operation(object):
   def __init__(self, shape, tool, paths, props, tabs=None):
      ...
      if tabs:
         ...
         self.tabs = Tabs([])
   def to_gcode(self, gcode, safe_z):
...
class HelicalDrill(Operation):
   ...
   def to_gcode(self, gcode, safe_z):
      if self.d < 2 * self.tool.diameter * self.tool.stepover:
         self.to_gcode_ring(gcode, safe_z, self.d)
      else:
         d = 2 * self.tool.diameter * self.tool.stepover
         while d < self.d:
            self.to_gcode_ring(gcode, safe_z, d)
            d += self.tool.diameter * self.tool.stepover
         self.to_gcode_ring(gcode, safe_z, self.d)
      gcode.rapid(z=safe_z)
         
   def to_gcode_ring(self, gcode, safe_z, d):
...
def gcodeFromOperations(operations, safe_z):
   gcode = Gcode()
   gcode.reset()
   gcode.rapid(z=safe_z)
   gcode.rapid(x=0, y=0)
   for operation in operations:
      operation.to_gcode(gcode, safe_z)
   gcode.rapid(x=0, y=0)
   gcode.finish()
   return gcode
