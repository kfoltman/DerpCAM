...
class EndMillPreset(object):
    def __init__(self, id, name, toolbit, hfeed, vfeed, maxdoc, stepover):
        self.id = IdSequence.register(id, self)
        self.name = name
    ...
    def description(self):
        return f"{self.name}: Fxy{self.hfeed:0.0f} Fz{self.vfeed:0.0f} DOC{self.maxdoc:0.2f} SO{100*self.stepover:0.0f}%"
...
class EndMillCutter(CutterBase):
    def __init__(self, id, name, material, diameter, length, flutes):
        ...
        self.flutes = flutes
    def addPreset(self, id, name, hfeed, vfeed, maxdoc, stepover):
        self.presets.append(EndMillPreset(id, name, self, hfeed, vfeed, maxdoc, stepover))
        return self
    def description(self):
    ...
    def __init__(self):
        ...
        self.toolbits = [
            EndMillCutter(1, "cheapo 2F 3.2/15", CutterMaterial.carbide, 3.2, 15, 2)
                .addPreset(100, "Wood-roughing", 3200, 1500, 2, 0.6)
                .addPreset(101, "Wood-finishing", 1600, 1500, 1, 0.6),
            EndMillCutter(2, "cheapo 2F 2.5/12", CutterMaterial.carbide, 2.5, 12, 2)
                .addPreset(102, "Wood-roughing", 3200, 1500, 2, 0.6)
                .addPreset(103, "Wood-finishing", 1600, 1500, 1, 0.6),
            EndMillCutter(3, "cheapo 1F 3.2/15", CutterMaterial.carbide, 3.2, 15, 1)
                .addPreset(104, "Alu-risky", 500, 100, 0.5, 0.4),
